...
configurations. We give the structural cause --- the bit-vector formula for $(x \cdot M) \gg k$
...
\end{tabular}
\end{center}
Solve time is flat up to about $2^{28}$ and then grows by roughly a factor of six per two added bits
($7.3 \to 27.9 \to 183.6$\,s), crossing the timeout between $2^{34}$ and $2^{36}$ --- exponential in the
live input width, not in the modulus or the formula size. At $x < 2^{46}$ the goal also failed to
