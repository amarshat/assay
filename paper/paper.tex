...
full product domain $x < 2^{46}$ fails to complete across six solver and tactic configurations, and a
single NTT layer fails likewise. We give the structural cause --- the bit-vector formula for $(x \cdot M) \gg k$
becomes intractable for current solvers as the live input width grows --- and three proof
restructurings that target it. One we carry to completion: a total-coefficient-invariant induction that
replaces a $\sim$3000-obligation SAW unrolling with an eight-step proof running in seconds. Two we
identify as the route past the remaining walls: an unbounded-integer reformulation with a bitvector
bridge, and a compositional specification. We also report
...
\begin{enumerate}
  ...
  \item \textbf{The structural cause.} The wall is the bit-vector formula for $(x \cdot M) \gg k$ ---
  a constant multiply and a power-of-two shift related to $x \bmod Q$ --- which becomes intractable for
  current solvers as the live input width grows; the same shape recurs in Barrett reduction and,
  256-fold, in one NTT layer (Section~\ref{sec:boundary}). This locates the boundary in the goal shape
  and lets us account for where it falls.
  ...
  induction running in seconds (Section~\ref{sec:ntt}); this escape we carry to completion. Two further
  restructurings --- an unbounded-integer reformulation with a bitvector bridge for the reductions, and
  a compositional specification for the layer proof --- we identify as the route past the remaining
  walls but do not mechanize here (Section~\ref{sec:boundary}).
...
\begin{tikzpicture}[
  ...
  step/.style={-{Latex[length=2mm]}, thick},
  ...
  \node[proven, below=of imod] (cspec) {FIPS 204 spec\\\scriptsize(C $\equiv$ spec, proven)};
  \draw[step] (cref) -- node[lbl, right] {hand-translate \;/\; SAW: C $\equiv$ Cryptol} (cmod);
  \draw[step] (cmod) -- node[lbl, right] {\code{cryptol-to-isabelle}} (imod);
  \draw[step] (imod) -- node[lbl, right] {Isabelle: model $\equiv$ FIPS} (cspec);
  ...
  \node[proven, below=of mir] (rspec) {Cryptol spec\\\scriptsize(Rust $\equiv$ spec, proven)};
  \draw[step] (rcrate) -- node[lbl, right] {\code{mir-json}} (mir);
  \draw[step] (mir) -- node[lbl, right] {SAW: Rust $\equiv$ Cryptol} (rspec);
...
exponential in the live input width, not in the modulus or the formula size. At $x < 2^{46}$ the goal also failed to
...
same 300\,s timeout). The corresponding narrow-domain Barrett reduction over \code{u32} inputs, used
elsewhere in ML-DSA, proves in seconds (Section~\ref{sec:rust}); the difference is the live input width
alone.

\paragraph{The structural cause.} The decisive subterm is $(x \cdot M) \gg 46$: a product up to roughly
$2^{69}$ divided by a power of two and related back to $x \bmod Q$. The bit-vector \emph{formula} is not
...
the full $2^{46}$ domain none of those bits are fixed, and the instance becomes intractable for every
back-end we tried. The boundary is therefore a property of the goal, not the solver: it appears wherever
a wide-domain $(x \cdot M) \gg k$ must be related to $x \bmod Q$ at bit-vector width, and it falls where
the live input width stops collapsing the instance --- here between $2^{34}$ and $2^{36}$, with solve
time growing exponentially in that width.
...
obligations for wide modular arithmetic can be hard to discharge is part of verification practice; what
has not been done, to our knowledge, is to measure where the boundary falls for a specific standardized
primitive, show it is a property of the goal rather than the toolchain by reproducing it across two of
them, and pair the measurement with a completed escape and two identified ones. That measurement and
those escapes are the contribution.
...
tractable, measured its steep onset, identified its structural cause, and gave one proof restructuring
that crosses it together with two more that we identify but do not yet mechanize. The same boundary
appears in both toolchains, which locates it in the goal rather than the tool. Mechanizing the
